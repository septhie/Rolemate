...
export default function FreeCreditsBar() {
  ...
  return (
    <div className="border-b border-white/8 bg-[#0d1117]/84 backdrop-blur-2xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-2 text-[11px] uppercase tracking-[0.26em] text-white/58 sm:px-6 lg:px-8">
        <span>{unlocked ? "Guest mode unlocked" : "Free Credits"}</span>
            ...
            {[0, 1, 2].map((index) => (
              <span
                ...
                className={`h-2.5 w-8 rounded-full transition ${
                  unlocked || index < remaining ? "bg-[#d4a85c] shadow-[0_0_24px_rgba(212,168,92,0.45)]" : "bg-white/12"
                }`}
              />
            ))}
          </div>
